...
| --- | --- | --- | --- |
| accordion | `examples/widget-gallery/src/accordion.rs` | New | Single-open disclosure composition now covers open/closed trigger, disabled trigger, focus ring, border-separated items, chevron swap, and shadcn content spacing. Reusable Accordion API and real height animation pending. |
| alert-dialog | `examples/widget-gallery/src/alert_dialog.rs` | New | Static shadcn-like alert dialog coverage now includes overlay preview, default/sm sizes, media and no-media headers, title/description, muted bordered footer, cancel/action buttons, and destructive action styling. Full modal/focus behavior pending. |
...
| bubble | `examples/widget-gallery/src/bubble.rs` | New | Message bubble variants default/secondary/muted/tinted/outline/ghost/destructive plus reactions. |
| button-group | `examples/widget-gallery/src/button_group.rs` | New | Horizontal, vertical, text, and separator compositions. |
...
| calendar | `examples/widget-gallery/src/calendar.rs` | New | Static DayPicker-style calendar coverage now includes caption label/dropdown styling, nav buttons, weekday row, outside/today/selected/range-start/range-middle/range-end/focused/disabled/hidden day states, state samples, and multi-month layout. Real DayPicker behavior pending. |
...
| hover-card | `examples/widget-gallery/src/hover_card.rs` | New | Tooltip-backed triggers and static shadcn-like hover-card surfaces now cover profile, repository, file, and team content with popover ring/shadow styling. Positioned HoverCard overlay API, open/close delay, and side-aware animation pending. |
...
| spinner | `examples/widget-gallery/src/spinner.rs` | New | Lucide loader-circle with rotate animation and sizes. |
...
| tabs | `examples/widget-gallery/src/tabs.rs` / `src/style/theme.rs` | Partial | Tab selector selected state now has shadcn-like shadow and not-allowed disabled cursor; gallery covers default, line, icons, disabled trigger, and vertical line orientation. |
