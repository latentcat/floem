...
impl<T: Clone + std::cmp::PartialEq + std::fmt::Debug> Dropdown<T> {
        ...
        T: std::fmt::Display,
    {
        const CHEVRON_DOWN: &str = r##"
            <svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="-46.336 -46.336 278.016 278.016">
                <path fill="currentColor" d="M92.672 144.373a10.707 10.707 0 0 1-7.593-3.138L3.145 59.301c-4.194-4.199
    ...
    fn create_overlay(&mut self) {
        ...
        exec_after_animation_frame(move |_| {
            // We need to request focus once the list has been styled and made visible or else it will not be considered focusable.
            if list_id.try_root().is_some() {
